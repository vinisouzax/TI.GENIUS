...
<head>
	...
	<script>
        ...
        function adicionaLinha(idTabela) {
        	...
        	celula2.innerHTML = "<select class='form-control' id='Variavel"+(numeroLinhas-1)+"' name='Variavel"+(numeroLinhas-1)+"' onchange='setaValores(this.value, this.id);' required></select>"; 
        	celula3.innerHTML = "<p></p>=";
        	celula4.innerHTML = "<select class='form-control' id='Valor"+(numeroLinhas-1)+"' name='Valor"+(numeroLinhas-1)+"' required></select>"; 
        	...
        	var select1 = document.getElementById("Variavel"+(numeroLinhas-1));
        	...
        	var select2 = document.getElementById("Valor"+(numeroLinhas-1));
        ...
        function confirma_excluir(idRegraXVariavel){
        	var apagar = confirm('Você deseja excluir esta condição');
        	if (apagar){
        		location.href = '<?php echo base_url("Regras/DeleteRegraXVariavel") ?>/'+ idRegraXVariavel;
        	}
        }       
    </script> 
</head>
				...
				<div class="login-wrapper">
					<div class="box">
						<div class="content-wrap">
							<h6>Edição de Regras <br><button style="border: none;" type="button" class="btn btn-default btn-lg" data-toggle="modal" data-target="#myModal"><i class="glyphicon glyphicon-info-sign"></i> Info</button></h6>
							...
							<div class="modal fade" id="myModal" role="dialog">
								...
								<?php if($nivelAcesso == "Administrador"){
									...
									echo form_open_multipart('Regras/UpdateRegra'); ?>
									<input class="form-control" type="hidden" id="Id" name="Id" value="<?php if($regra->idRegra != ''){ echo $regra->idRegra;}  ?>">
									<p style="font-size: 14px;"><strong>Nome da Regra</strong></p>
									<input maxlength="50" class="form-control" type="text" id="Nome" name="Nome" value="<?php if($regra->nmRegra != ''){ echo $regra->nmRegra;}  ?>" placeholder="Nome da Regra" required>
									<p style="font-size: 14px;"><strong>Categoria do Problema</strong></p>
									<select class="form-control col-md-12" id="Categoria" name="Categoria" required>
										<option value="">Selecione a categoria</option>
										<?php foreach ($categorias as $categoria ) { ?>
											<option value="<?= $categoria->idCategoria; ?>" <?=($categoria->idCategoria == $regra->idCategoria)? 'selected':''?>> <?= $categoria->nmCategoria; ?> </option>
										<?php  }  ?>
									</select>
									<br><br><br>
									<p style="font-size: 14px;"><strong>Solução (pdf)</strong></p>
									<!-- <p style="font-size: 14px;"><?php //echo str_replace(base_url().'tutoriais/', '', $regra->solucao); ?></p>-->
									<input class="form-control" type="file" id="solucao" name="solucao"></input>
									<br><br><br>
									...
									<div class="table-responsive">
										<table class="table" id="tabela" name="tabela">
											...
											<tbody class="col-md-12">
												<?php
												$i = 0;
												foreach ($regrasxvariaveis as $regraxvariavel) { 
													if($regraxvariavel->idRegra == $regra->idRegra){
														echo '<tr class="odd gradeX">';
														echo '<td hidden><input class="form-control" type="text" id="RegraXVariavel' . $i . '" name="RegraXVariavel' . $i . '" value="' . $regraxvariavel->idRegraXVariavel . '"></td>';
														echo '<td><p></p>SE</td>';
														echo "<td><select class='form-control' id='Variavel". $i ."' name='Variavel". $i ."' onchange='setaValores(this.value, this.id);' required>";
														foreach($variaveis as $variavel){ ?>
															<option value="<?= $variavel->idVariavel; ?>" <?=($variavel->idVariavel == $regraxvariavel->idVariavel)? 'selected':''?>> <?= $variavel->nmVariavel; ?> </option>
														<?php }
														echo "</select></td>"; 
														echo "<td><p></p>=</td>";
														echo "<td><select class='form-control' id='Valor". $i ."' name='Valor". $i ."' required>";
														foreach($valores as $valor){ 
															if($valor->idVariavel == $regraxvariavel->idVariavel){ ?>
																<option value="<?= $valor->idValor; ?>" <?=($valor->idValor == $regraxvariavel->idValor)? 'selected':''?>> <?= $valor->nmValor; ?> </option>
															<?php } }                                               
															echo "</select></td>";
															echo '<td> <a class="btn btn-danger btn-xs" onclick="confirma_excluir(' . $regraxvariavel->idRegraXVariavel . ');"><i class="glyphicon glyphicon-remove"></i> Remover </a></td>';
															echo '</tr>';
															$i++;
														}
													}
													echo '<input class="form-control" type="hidden" id="qtd" name="qtd" value="' . $i . '">';
